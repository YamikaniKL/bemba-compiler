...
class BembaTransformer {
    ...
    transformComponent(node) {
        const reactComponent = {
            ...
            stateWithEffects: this.transformStateWithEffects(node.stateWithEffects),
    ...
    transformStateWithEffects(stateWithEffects) {
        if (!stateWithEffects || typeof stateWithEffects !== 'object') {
            return {};
        }
        
        const reactStateWithEffects = {};
        for (const [key, value] of Object.entries(stateWithEffects)) {
            if (key === 'effect') {
                reactStateWithEffects.effect = this.transformNode(value);
            } else {
                reactStateWithEffects[key] = this.transformNode(value);
            }
        }
        
        return reactStateWithEffects;
    }
    ...
    transformFunctionCall(node) {
        const functionMappings = {
            ...
            // State management
            [BEMBA_SYNTAX.UPDATE_STATE]: 'setState',
            
            // Built-in functions
            [BEMBA_SYNTAX.FUNCTIONS.console.log]: 'console.log',
            [BEMBA_SYNTAX.FUNCTIONS.console.error]: 'console.error',
            [BEMBA_SYNTAX.FUNCTIONS.console.warn]: 'console.warn',
            [BEMBA_SYNTAX.FUNCTIONS.string.length]: 'length',
            [BEMBA_SYNTAX.FUNCTIONS.string.replace]: 'replace',
            [BEMBA_SYNTAX.FUNCTIONS.string.split]: 'split',
            [BEMBA_SYNTAX.FUNCTIONS.string.join]: 'join',
            [BEMBA_SYNTAX.FUNCTIONS.array.push]: 'push',
            [BEMBA_SYNTAX.FUNCTIONS.array.splice]: 'splice',
            [BEMBA_SYNTAX.FUNCTIONS.array.map]: 'map',
            [BEMBA_SYNTAX.FUNCTIONS.array.filter]: 'filter',
            [BEMBA_SYNTAX.FUNCTIONS.math.add]: '+',
            [BEMBA_SYNTAX.FUNCTIONS.math.subtract]: '-',
            [BEMBA_SYNTAX.FUNCTIONS.math.multiply]: '*',
            [BEMBA_SYNTAX.FUNCTIONS.math.divide]: '/'
        };
        
        const reactFunctionName = functionMappings[node.name] || node.name;
        
        // Special handling for state updates
        if (node.isStateUpdate) {
            return {
                type: 'StateUpdate',
                stateName: node.args[0],
                newValue: node.args[1],
                args: node.args.map(arg => this.transformNode(arg))
            };
        }
