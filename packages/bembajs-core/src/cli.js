...
`;
    ...
    async startViteDevServer(options, configFile) {
        ...
        const injiniLogger = createBembaInjiniLogger(
            createLogger('info', { allowClearScreen: true, prefix: '[Phisha]' }),
            process.cwd()
        );
  ...
  <script>
    ...
    async buildProject(options) {
        ...
        if (viteConfig) {
            ...
            const injiniBuildLogger = createBembaInjiniLogger(
                createLogger('info', { allowClearScreen: true, prefix: '[Phisha]' }),
                process.cwd()
            );
            const glue = this.ensureManagedInjiniGlue(process.cwd());
...
`;
    ...
    async exportStatic(options) {
        ...
        if (viteConfig) {
            ...
            const { build, createLogger } = await import('vite');
            const { createBembaInjiniLogger } = require('./injini-vite-messages');
            const injiniExportLogger = createBembaInjiniLogger(
                createLogger('info', { allowClearScreen: true, prefix: '[Phisha]' }),
                process.cwd()
            );
            const glue = this.ensureManagedInjiniGlue(process.cwd());
